...
front/build:
	cd front && env REACT_APP_REVISION=`git rev-parse HEAD` DISABLE_ESLINT_PLUGIN=true yarn build

front/deploy:
	rsync -auv --delete-after front/build/ front/_app/

front-k8s/build:
	cd front && env REACT_APP_REVISION=`git rev-parse HEAD` DISABLE_ESLINT_PLUGIN=true PUBLIC_URL=/ REACT_APP_BASENAME= yarn build
